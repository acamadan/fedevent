...
<!-- BUILD: CREATA-SUITE 2025-08-11 v2 -->
<style>
...
<div class="wrap">
  <div class="card">
    ...
    <form id="hotel-form" method="POST" enctype="multipart/form-data">
      ...
      <!-- PAGE 0: Start (Guest / Account / Sign-In) -->
      ...
      <div class="page" data-page="1">
        <fieldset>
          ...
          <div class="item">
            ...
            <div class="hint">Best results with DOCX/PPTX or text-based PDFs. Scanned PDFs require OCR enabled on the API.</div>
          </div>
        ...
        <fieldset>
          ...
          <div class="row">
            <div class="col-4">
              <label>Accepts NET30 Terms from CREATA (Prime)? <span class="required">*</span></label>
            ...
            <div class="col-12">
              <div class="inline"><input id="terms_ack" type="checkbox" required />
                <label for="terms_ack" style="font-weight:500">I acknowledge that <strong>NET30</strong> is the payment term. Hotel will invoice CREATA (prime) per PO.</label>
              </div>
            </div>
          </div>
        </fieldset>
      ...
      <div class="page" data-page="2">
        <fieldset>
          ...
          <div class="row">
            ...
            <div class="col-6"><label>F&amp;B Minimums (by space or day)</label><input name="fb_minimums" placeholder="e.g., $15,000/day in ballroom" /></div>
        ...
        <fieldset>
          ...
          <div class="row">
            ...
            <div class="col-4">
              <label>Childcare via Third-Party Provider? <span class="required">*</span></label>
            ...
            <div class="col-12"><label>Childcare Provider Details / Requirements</label><input name="childcare_notes" placeholder="Providers, certifications, hours, ratios" /></div>
...
<script>
  const API_BASE = 'https://api.creataglobal.com';
  ...
  let current = 0, submittingFlag = false;
  ...
  // Optional account endpoints (safe if not present)
  if (btnSignup) btnSignup.addEventListener('click', async ()=>{
    ...
    try{
      ...
      const resp = await fetch(API_BASE+'/api/signup', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({email:user.includes('@')?email:null, username:user, password:pass, account_email:email})});
      const j = await resp.json().catch(()=>({}));
      if (!resp.ok || !j.ok){ showToast(j.error||'Sign up not available yet. Continuing as guest…'); }
      else { showToast('Account created. Please verify via email.'); }
    }catch(e){ showToast('Signup unavailable. Continuing as guest.'); }
  ...
  if (btnSignin) btnSignin.addEventListener('click', async ()=>{
    ...
    try{
      submitting.classList.add('show');
      const resp = await fetch(API_BASE+'/api/signin', {method:'POST', headers:{'Content-Type':'application/json'}, credentials:'include', body: JSON.stringify({user, password:pass})});
      const j = await resp.json().catch(()=>({}));
      if (!resp.ok || !j.ok){ showToast(j.error||'Sign in failed. You can continue as guest.'); }
      else { showToast('Signed in.'); }
    }catch(e){ showToast('Sign in unavailable. You can continue as guest.'); }
    finally{ submitting.classList.remove('show'); }
  });

  if (forgotLink) forgotLink.addEventListener('click', (e)=>{
    e.preventDefault(); showToast('Password reset link will be sent if account exists.');
  });
  ...
  btnAutofill.addEventListener('click', async ()=>{
    const f = autofillFile.files && autofillFile.files[0];
    if (!f) { showToast('Choose a file first.'); return; }
    const fd = new FormData(); fd.append('info_sheet', f);
    try {
      submitting.classList.add('show');
      const resp = await fetch(API_BASE + '/api/autofill', { method:'POST', body: fd, credentials:'include' });
      let json;
      try { json = await resp.json(); } catch(parseErr){
        const text = await resp.text();
        throw new Error('Autofill parse error: ' + text.slice(0,180));
      }
  ...
  form.addEventListener('submit', async (e)=>{
    ...
    try {
      ...
      const resp = await fetch(API_BASE + '/api/submit', { method:'POST', body: fd, credentials:'include' });
      let json;
      try { json = await resp.json(); } catch(parseErr){
        const text = await resp.text();
        throw new Error('Submit parse error: ' + text.slice(0,200));
      }
